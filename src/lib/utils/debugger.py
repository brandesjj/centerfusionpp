...
# debug
from .ddd_utils import rot_y2alpha
...
class Debugger(object):
  ...
  def plot_radar2img(self, img_id, img, batch, hmap_str='depth'): 

      self.imgs[img_id] = copy.copy(img)

      pc_ef = batch['pc_ef'].squeeze().detach().cpu().numpy()
      overlay_indices = np.nonzero(pc_ef[0,:,:])
      
      # Calculate color depending on depth
      hmin=1
      hmax=240

      if len(overlay_indices[0]) > 0:
        # Only plot if there is something to plot
        if hmap_str == 'depth':
          # Shift by minimum
          hmap_colors = pc_ef[0,:,:][overlay_indices]
          hmap_colors = hmap_colors - np.amin(hmap_colors)
          hmap_colors = (hmax-hmin)/(np.amax(hmap_colors)+1e-5)*hmap_colors
          hmap_colors += hmin
          color_array = np.array([0,hmin,200])*np.ones(self.imgs[img_id][overlay_indices].shape)
          color_array[:,1] = hmap_colors
        
        else:
          color_array = np.array([0,165,200])*np.ones(self.imgs[img_id][overlay_indices].shape)

        # Color in orange
        self.imgs[img_id][overlay_indices] = color_array
  ...
  def add_overlay_img(self, img, fore, img_id="overlay", alpha=0.1):
    # Darker corresponds to smaller value in "fore"
    self.imgs[img_id] = img.copy()
    fore = fore.copy()
    ...
    color_map = (fore * colors).max(axis=2)
    # Scale color map values between 0 and 255
    color_scale_factor = 255/(color_map.max()+1e-5)
    color_map = (color_map*color_scale_factor).astype(np.uint8)
    ...
    for ii, (dets, lc, cc) in enumerate(
      ...
      for item in dets:
        if item['score'] > vis_thresh \
          ...
          for e in [[0, 1]]:
            t = 4 if e == [0, 1] else 1
            cv2.line(bird_view, (np.int_(rect[e[0]][0]), np.int_(rect[e[0]][1])),
                    (np.int_(rect[e[1]][0]), np.int_(rect[e[1]][1])), lc, t,
                    lineType=cv2.LINE_AA)
          if show_velocity:
