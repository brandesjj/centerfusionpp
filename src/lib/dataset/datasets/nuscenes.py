...
class nuScenes(GenericDataset):
  ...
  def __init__(self, opt, split, eval_frustum=None):
    split_names = {
        ...
        'nano_train':'nano_train', 
        'nano_val':'nano_val',    
        'night_and_rain_val':'night_and_rain_val',
        'night_rain_val':'night_rain_val',
        'night_val':'night_val',
    }
    ...
    data_dir = opt.data_dir # Changed due to change in file structure for cluster training
    anno_dir = os.path.join(opt.data_dir, 'annotations')
    ...
    anns_dir += opt.annotation_dir_ending
    
    if opt.dataset_version == 'test':
      ann_path = os.path.join(anno_dir, anns_dir, 'test.json')
    else:
      ann_path = os.path.join(anno_dir, anns_dir, '{}.json').format(split_name)

    self.images = None
    # Get img ids and save to self.images as well as set the getitem func
    super(nuScenes, self).__init__(opt, split, ann_path, data_dir, eval_frustum)
    ...
    for image_id in all_bboxes:
      if type(all_bboxes[image_id]) != type({}):
        ...
        for j in range(len(all_bboxes[image_id])):
          ...
          category_id = citem['class'] # why is this citem? Shouldn't it be item?
          bbox = item['bbox']
          bbox[2] -= bbox[0]
          bbox[3] -= bbox[1]
  ...
  def convert_eval_format(self, results):
    ...
    if self.opt.iou_thresh > 0:
      ...
      for sample_token, dets in tqdm(ret['results'].items()):
        ret['results'][sample_token], n = self.apply_bev_nms(dets, self.opt.iou_thresh, dist_thresh=2)
        n_removed += n
  ...
  def run_eval(self, results, save_dir, n_plots=10, render_curves=False):
    # save_dir is the directory of the 'logs_<timestamp>' folder in this case
    task = 'tracking' if self.opt.tracking else 'det'
    split = self.opt.val_split
    # if split == 'tiny_val':
    #   # There is no tiny_val set which the eval script from nuScenes could use
    #   split = 'mini_val'
    # version = 'v1.0-mini' if ('mini' in split or 'tiny' in split) else 'v1.0-trainval'
    if split in self.version_dict.keys():
      version = self.version_dict[split]
    else:
      version = 'v1.0-trainval'
    self.save_results(results, save_dir, task, split)
    render_curves = 1 if render_curves else 0
    data_path = self.opt.data_dir
    
    if task == 'det':
      # Detection task
      print(f'Executing detection task. Dataroot is {data_path + "/nuscenes/"}')
      output_dir = '{}/nuscenes_eval_det_output_{}/'.format(save_dir, split)
      # Run evaluation script using system python
      os.system('python3 ' + \
        '{}'.format(self.opt.root_dir) + \
        ...
        '--dataroot {} ' .format(data_path + '/nuscenes/') + \
        '--version {} '.format(version) + \
        '--plot_examples {} '.format(n_plots) + \
        '--render_curves {} '.format(render_curves))
    else:
      # Tracking task
      print(f'Executing tracking task. Dataroot is {data_path + "/nuscenes/"}')
