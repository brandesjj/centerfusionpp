...
def get_prediction_challenge_split(split: str, dataroot: str = '/data/sets/nuscenes') -> List[str]:
    ...
    """
    if split not in {'mini_train', 'mini_val', 'train', 'train_val', 'val', 'tiny_train', 'tiny_val'}:
        raise ValueError("split must be one of (mini_train, mini_val, train, train_val, val, tiny_train, tiny_val)")
