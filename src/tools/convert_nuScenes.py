...
Based on Nabatis work, but extended by some additional filters and data-splits.
'''
import os
import json
import posix
...
OUT_PATH = '../../data/annotations/annotations'
...
# The default datasets are mini and full (train,val,test) size.
# tiny_(..) contains only 1 scene, wee_(..) only 2 samples
# night_(..) only night scenes
# night_rain_(..) night scenes plus rain scenes (might not be at night)
# night_and_rain_(..) night scenes with rain
...
SPLITS = {
  ...
  # CUSTOM
          ...
          # 'wee_train': 'v1.0-mini',
          # 'nano_val': 'v1.0-mini',
          # 'nano_train': 'v1.0-mini',
          # 'night_rain_val': 'v1.0-trainval',
          # 'night_val': 'v1.0-trainval',
          # 'night_train': 'v1.0-trainval',
          # 'night_rain_train': 'v1.0-trainval',
          # 'night_and_rain_val': 'v1.0-trainval',
          # 'night_and_rain_train': 'v1.0-trainval',
          }

# Activate debugging (show images with matplotlib)
DEBUG = False

# Do not keep the bounding boxes that are not visible in a picture when True
# (default CenterFusion++: False)
...
# Thresholds are defined by nuScenes.
# 1 - visibility 0-40%    2 - visibility 40-60%    3 - visibility 60-80%    4 - visibility 80-100%
visiblity_threshold = 2

# Activates the custom radar data filtering 
CUSTOM_RADAR_FILTER = False # default CenterFusion++: False
...
PREPROCESSING_NUSCENES = False # default CenterFusion++: False
...
# Camera Sensors that should be used:
...
OUT_PATH = OUT_PATH + suffix1 + suffix2 + suffix3 + '/'
...
def main():
  ...
  for split in SPLITS:
    ...
    # Number of samples we want to take from each scene
    if split in ['nano_train', 'nano_val']:
      num_max_samples = 1
    elif split in ['wee_train', 'wee_val']:
      num_max_samples = 2
    else:
      # Use all samples
      num_max_samples = []
    ...
    for sample in nusc.sample:
      # A "sample" in nuScenes refers to a timestamp with 6 cameras, 1 LIDAR and 5 radars).
      ...
      if num_max_samples != [] and num_images >= num_max_samples*len(USED_SENSOR):
        # We only want a very limited number of samples in the wee dataset
        # Therefore, skip this sample in case we already have the number of samples we need
        continue
      ...
      for sensor_name in sample['data']:
        if sensor_name in USED_SENSOR:
          ...
          # BoxVisibility.ANY requires all corner of the image to be within the image
          ...
          # Add another channel for the timestamp. Not possible in init of RadarPointCloud (pre-defined)
          all_radar_pcs.points = np.empty((19,0))
          for radar_channel in RADARS_FOR_CAMERA[sensor_name]:
            if CUSTOM_RADAR_FILTER:
              # Activate custom radar point filtering
              radar_pcs, radar_delta_ts = RadarPointCloud.from_file_multisweep(nusc, 
                sample, radar_channel, sensor_name, NUM_SWEEPS,
                invalid_states=invalid_states_custom, dynprop_states=dynprop_states_custom)
            else: # Use custom radar point filtering
              # radar_delta_ts is the time difference of the sample reference to the sweep of the reference
              # The latest radar points have the biggest values.
              radar_pcs, radar_delta_ts = RadarPointCloud.from_file_multisweep(nusc, 
                sample, radar_channel, sensor_name, NUM_SWEEPS)
            # Stack delta timestamp as 18th signal
            radar_pcs.points = np.vstack((radar_pcs.points, radar_delta_ts))
          ...
          if DEBUG:
            # Plot radar point cloud and corresponding radial velocities
            pcl = image_info['radar_pc']
            vx = pcl[8]
            vz = pcl[9]
            posx = pcl[0]
            posz = pcl[2] 

            plt.scatter(0,0,20,marker='x',color='r') # origin Camera CS

            for i in np.arange(len(vx)):
              plt.scatter(posx[i], posz[i], 1.5, color='g')
              plt.arrow(posx[i],posz[i],vx[i],vz[i],color='r')
              plt.axline((posx[i]+vx[i],posz[i]+vz[i]),(posx[i],posz[i]),color='r',alpha=0.1)
            
            plt.grid()
            plt.show() 

          # Sort boxes by depth
          boxes_sorted = sorted(boxes, key=sort_depth, reverse=True)

          for box in boxes_sorted: # Iterate over all BB for the current image
            
            # Skip if there is neither a radar nor a lidar point in the annotation
            if PREPROCESSING_NUSCENES:
              if (nusc.get('sample_annotation', box.token)['num_lidar_pts'] == 0 
                    and nusc.get('sample_annotation', box.token)['num_radar_pts'] == 0):
                continue
            # Get visibility level from nuScenes
            # 1 - visibility 0-40%    2 - visibility 40-60%    3 - visibility 60-80%    4 - visibility 80-100%
            visibility_level = nusc.get('sample_annotation', box.token)['visibility_token']
            if SKIP_NON_VISIBLE_ANNOTATIONS and int(visibility_level) < visiblity_threshold:
              # Skip the current annotation and therefore do NOT append it to the anns list
              continue
            ...
            # Project center of bounding box to image to find amodel_center
            ...
            bbox = KittiDB.project_kitti_box_to_image( # KITTI bbox format: (x1,y1,x2,y2) where 1 and 2 correspond the opposing corners 
            ...
            ann['bbox'] = [bbox[0], bbox[1], bbox[2] - bbox[0], bbox[3] - bbox[1]] # New bbox format: (x,y,width,height)
