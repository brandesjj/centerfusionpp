...
from ...src.lib.utils.pointcloud import RadarPointCloud
...
SPLITS = {
        ...
        #   'mini_val': 'v1.0-mini',
        #   'Training': 'v1.0-trainval',
        #   'Validation': 'v1.0-trainval',
        #   'Test': 'v1.0-test',
          'tiny_val': 'v1.0-mini',
        #   'tiny_train': 'v1.0-mini',
          }
...
def main():
    ...
    day_scenes = dict()
    rain_scenes = dict()
    night_and_rain_scenes = dict()
    night_or_rain_scenes = dict()
    num_cats = dict()
    num_night_scenes = dict()
    num_day_scenes = dict()
    num_rain_scenes = dict()
    cats_radar_points = dict()
    cats_lidar_points = dict()
    max_rcs = -1e4
    min_rcs = 1e4
    ...
    for split in SPLITS:
        try:
            print(f'Split {split} contains {len(SCENE_SPLITS[split])} scenes.')
        except:
            pass
        ...
        night_scenes[split] = list()
        day_scenes[split] = list()
        rain_scenes[split] = list()
        night_and_rain_scenes[split] = list()
        
        for scene in nusc.scene:
            # Don't continue if scene name is not in SCENE_SPLITS (exception: testset)
            if not (split in ['Test']) and not scene['name'] in SCENE_SPLITS[split]:
                continue
            if 'night' in scene['description'].lower():
                # night_scenes[split] = [night_scenes[split], scene['name']]
                # night_scenes[split] = copy.copy(night_scenes[split]).append(scene['name'])
                night_scenes[split].append(scene['name'])
            else:
                # day_scenes[split] = [day_scenes[split], scene['name']]
                day_scenes[split].append(scene['name'])
            if 'rain' in scene['description'].lower():
                # rain_scenes[split] += 1
                rain_scenes[split].append(scene['name'])
                # rain_scenes[split] = [rain_scenes[split], scene['name']]

        num_night_scenes[split] = len(night_scenes[split])
        num_day_scenes[split] = len(day_scenes[split])
        num_rain_scenes[split] = len(rain_scenes[split])

        # Get night and rain scenes:
        night_and_rain_scenes[split] = list(set(night_scenes[split]).intersection(rain_scenes[split]))
        night_or_rain_scenes[split] = list(set(night_scenes[split]+rain_scenes[split]))

        num_cats[split] = dict.fromkeys(CATS, 0)
        cats_radar_points[split] = defaultdict.fromkeys(CATS, [])
        cats_lidar_points[split] = dict.fromkeys(CATS, 0)
        # Iterate over all samples
        for sample in nusc.sample:
            scene_name = nusc.get('scene', sample['scene_token'])['name']
            if split == 'Test': # No annotations for Test split
                continue
            if not scene_name in SCENE_SPLITS[split]:
                continue
            # Get ego pose
            sample_data = nusc.get('sample_data', sample['data']['LIDAR_TOP'])
            ego_translation = np.array(nusc.get('ego_pose', sample_data['ego_pose_token'])['translation'])

            # Iterate over every annotation in the sample
            for ann_token in sample['anns']:
                ann = nusc.get('sample_annotation', ann_token)
                # Calculate distance to annotation
                dist = np.linalg.norm((ego_translation-ann['translation'])[0:2])
                # print(dist)
                # Save category
                category = category_to_detection_name(ann['category_name'])
                if category != None and dist < dist_threshold:
                    num_cats[split][category] += 1
                    # Save RADAR points
                    # cats_radar_points[split][category] = [cats_radar_points[split][category], ann['num_radar_pts']]
                    temp = copy.copy(cats_radar_points[split][category])
                    temp.append(ann['num_radar_pts'])
                    cats_radar_points[split][category] = temp
        
            # Get min / max RCS Value
            for radar in RADARS:
                pointcloud = RadarPointCloud
                radar_data = nusc.get('sample_data', sample['data'][radar])
                radar_pcs, radar_delta_ts = nusc.from_file(osp.join(nusc.dataroot, radar_data['filename'])) 
                                        

##############################################
# Plot bar plot of occurrences of categories in annotations of splits
    
    # fig, ax = plt.subplots(figsize = (5.9, 4))

    # plt.xticks(rotation=40, ha='right')

    # # using the twinx() to create another axes
    # ax2 = ax.twinx()
    # num_cats_keys = [*num_cats]

    # # creating a bar plot
    # sorted_dict1 = sorted(num_cats[num_cats_keys[0]].items(), key=operator.itemgetter(1),reverse=True)
    # sorted_dict2 = sorted(num_cats[num_cats_keys[1]].items(), key=operator.itemgetter(1),reverse=True)

    # ax.bar(*zip(*sorted_dict1), color = 'blue', align='edge', width=-0.3, label=num_cats_keys[0])
    # ax2.bar(*zip(*sorted_dict2), color = 'orange', align='edge', width=0.3, label=num_cats_keys[1])
    
    # # Label axes
    # ax.set_xlabel('Object Category')
    # ax.set_ylabel(f'Occurrences in {num_cats_keys[0]}')
    # ax2.set_ylabel(f'Occurrences in {num_cats_keys[1]}')
    
    # ax.legend(loc=0, bbox_to_anchor=(1,0.88))
...
##############################################
# Plot histogram of radar points for cars
    splits = [*SPLITS]

    # Calculate mean for all categories over all splits
    cats_radar_points_mean = dict()
    for cat in cats_radar_points[splits[0]]:
        arr = np.empty(0)
        for split in splits:
            arr = np.concatenate((arr, np.array(cats_radar_points[split][cat])))
        cats_radar_points_mean[cat] = np.mean(arr)
    
    fig_hist, ax_hist = plt.subplots(figsize = (5.9, 4))
    

    radar_data = np.array(cats_radar_points[splits[0]]['car'])

    nonzero_radar_data = radar_data[np.nonzero(radar_data)]

    plt.hist(nonzero_radar_data, 30)

## Plot mean radar points via category
    fig_radarmean, ax_radarmean = plt.subplots(figsize = (5.9, 4))
    plt.xticks(rotation=40, ha='right')

    cats = [*cats_radar_points_mean]

    # creating a bar plot
    sorted_dict = sorted(cats_radar_points_mean.items(), key=operator.itemgetter(1),reverse=True)
    sorted_values = []
    for i in range(len(sorted_dict)):
        sorted_values.append(np.around(sorted_dict[i][1],2)) 
    ax_radarmean.bar(*zip(*sorted_dict), color = 'blue')
    # for index, value in enumerate(sorted_values):
    #     plt.text(value, index, str(value))

    # # Label axes
    ax_radarmean.set_xlabel('Object Category')
    ax_radarmean.set_ylabel('Mean radar points per annotation')

    ax_radarmean.set_axisbelow(True)
    ax_radarmean.grid(True)
